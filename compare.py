...
def create_excel(reports: List[Report]):
    ...
    for project_name in dict_of_projects.keys():
        ...
        excel_handler.heatmap_sheet.write(dict_of_projects[project_name], 0,
                                          project_name, excel_handler.label_format)
        ...
        for row_idx, row in enumerate(table_of_reports):
            ...
            for col_idx, cell_value in enumerate(row):
                ...
                if isinstance(cell_value, int):
                    sheet.write(row_idx, table_width - 1, cell_value, self.get_format(cell_value))
                    break
                if is_first:
                ...
                if isinstance(cell_value, str) and column_lengths.get(col_idx, 0) < len(cell_value):
